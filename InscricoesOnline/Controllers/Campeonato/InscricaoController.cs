...
using InscricoesOnline.ModelView;
...
namespace InscricoesOnline.Controllers.Admin.Campeonato
{
    public class InscricaoController : Controller
    {
        ...
        [Route("Admin/Inscricao/Eventos")]
        public ActionResult Eventos()
        {
            var eventosInscricoes = new List<EventoInscricoes>();

            foreach (var evento in db.Eventos)
            {
                eventosInscricoes.Add(new EventoInscricoes
                {
                    Eventos = evento,
                    Inscricoes = db.Inscricoes.Where(i => i.EventoId == evento.Id).ToList()
                });
            }
            return View(eventosInscricoes.OrderByDescending(e => e.Eventos.DataInicio).ToList());
        }


        [Route("Admin/Inscricao/Lista/{id}")]
        public ActionResult Lista(long? id)
        {
            ...
            var eventoInscricoes = new EventoInscricoes
            {
                Eventos = evento,
                Inscricoes = db.Inscricoes.Include(i => i.Academia).Include(i => i.Evento).Include(i => i.Filiado).Where(i => i.Evento.Id == evento.Id).OrderBy(i => i.Academia.Nome).ThenBy(i => i.Filiado.Nome).ToList()
            };

            return View(eventoInscricoes);
        }
        ...
        [Route("Admin/Inscricao/ImprimirCertificado/{id}")]
        public ActionResult ImprimirCertificado(long? id)
        {
            if (id == null)
            {
                return new HttpStatusCodeResult(HttpStatusCode.BadRequest);
            ...
            var inscricaoModalidade = db.InscricoesModalidades.Where(i => i.InscricaoId == id).FirstOrDefault();
            return View(inscricaoModalidade);
        }

        [Route("Admin/Inscricao/Novo/{id}")]
        public ActionResult Novo(long? id)
        {
            if (id == null)
            {
                return new HttpStatusCodeResult(HttpStatusCode.BadRequest);
            ...
            ViewBag.Academias = db.Academias.Where(a => a.Ativo).OrderBy(a => a.Nome).ToList();
        ...
        public ActionResult NovoSalvar(InscricaoViewModel inscricaoViewModel, FormCollection formCollection)
        {
            ...
            {
                var incricoesModalidadesExcluidas = db.InscricoesModalidades.Where(i => i.Inscricao.AcademiaId == inscricaoViewModel.Academia.Id &&
                                                                                        i.Inscricao.EventoId == inscricaoViewModel.Evento.Id).ToList();

                incricoesModalidadesExcluidas.ForEach(e => db.Entry(e).State = EntityState.Deleted);

                var inscricoesExcluidas = db.Inscricoes.Where(i => i.AcademiaId == inscricaoViewModel.Academia.Id &&
                                                              i.EventoId == inscricaoViewModel.Evento.Id).ToList();
            ...
            if (!String.IsNullOrEmpty(formCollection["txtIdFiliado"]))
            {
                ...
                var incricoesModalidadesExcluidas = db.InscricoesModalidades.Where(i => i.Inscricao.AcademiaId == inscricaoViewModel.Academia.Id &&
                                                                                        i.Inscricao.EventoId == inscricaoViewModel.Evento.Id &&
                ...
                for (var i = 0; i < IdsFiliados.Count; i++)
                {
                    if (IdsInscricaoModalidade[i] == 0)
                    {
                        ...
                        var inscricao = db.Inscricoes.Where(ins => ins.EventoId == inscricaoViewModel.Evento.Id &&
                                                                   ins.AcademiaId == inscricaoViewModel.Academia.Id &&
                        ...
                        if (inscricao == null)
                        {
                            ...
                            inscricao.AcademiaId = inscricaoViewModel.Academia.Id;
                            inscricao.EventoId = inscricaoViewModel.Evento.Id;
                ...
                db.SaveChanges();
                db.Inscricoes.Where(i => i.AcademiaId == inscricaoViewModel.Academia.Id &&
                                         i.EventoId == inscricaoViewModel.Evento.Id &&
                                         i.InscricaoModalidade.Count == 0).ToList().ForEach(i => db.Entry(i).State = EntityState.Deleted);

                db.SaveChanges();
            }
            return RedirectToAction("Lista", new { id = inscricaoViewModel.Evento.Id });
        }
        ...
        public ActionResult SortearEvento(long? id)
        {
            ...
            var chaves = db.Chaves.Include(c => c.Inscritos).Include(c => c.ChaveParticipantes).Where(c => c.EventoId == id).ToList();

            foreach (var chave in chaves)
            {
                if (chave.Modalidade.Ranking)
                {
                    chave.Inscritos.ForEach(i => i.Inscricao.PontosRanking = db.Rankings.Where(r => r.FiliadoId == i.Inscricao.FiliadoId &&
                                                                                                    r.Ano == i.Inscricao.Evento.DataInicio.Value.Year &&
                                                                                                    r.CategoriaPesoRankingId == chave.CategoriaLutaPeso.CategoriaPesoRankingId)
                                                                                        .Sum(r => (decimal?)r.Pontos) ?? 0);

                    chave.Inscritos.ForEach(i => i.Inscricao.PontosPesoRanking = db.Rankings.Where(r => r.FiliadoId == i.Inscricao.FiliadoId &&
                                                                                                        r.Ano == i.Inscricao.Evento.DataInicio.Value.Year &&
                                                                                                        r.CategoriaPesoRankingId == chave.CategoriaLutaPeso.CategoriaPesoRankingId)
                                                                                            .Sum(r => (decimal?)r.RankingEvento.Peso * r.Pontos) ?? 0);

                    chave.Inscritos = chave.Inscritos.OrderByDescending(i => i.Inscricao.PontosRanking).ThenByDescending(i => i.Inscricao.PontosPesoRanking).ToList();
            ...
            var chavesAtualizadas = db.Chaves.Include(c => c.Inscritos).Include(c => c.ChaveParticipantes)
                                                            .Where(c => c.EventoId == id && c.Inscritos.Count() > 0)
                                                            .OrderBy(c => c.Modalidade.Titulo)
                                                            .ThenBy(c => c.CategoriaFaixa.FaixaInicial.Ordem)
                                                            .ThenBy(c => c.CategoriaIdade.IdadeInicial)
                                                            .ThenBy(c => c.Sexo)
            ...
            return View("GerarChaves", chaveViewModel);
        }
        ...
        private Stream ImprimirChave(Chave chave)
        {
            ...
            ((TextObject)rd.ReportDefinition.ReportObjects["txtNomeEmpresa"]).Text = db.Empresa.FirstOrDefault()?.Nome.ToUpper();
        ...
        public ActionResult Desclassificar(long? id)
        {
            ...
            var chaveParticipantes = db.ChaveParticipantes.Where(c => c.Chave.EventoId == id);
        ...
        [Route("Admin/Inscricao/Controle/{id}")]
        public ActionResult Controle(long? id)
        {
            if (id == null)
            {
                return new HttpStatusCodeResult(HttpStatusCode.BadRequest);
            ...
            var chave = db.Chaves.Where(c => c.EventoId == id);
            if (chave == null)
            {
                return HttpNotFound();
            }
        ...
        public ActionResult Classificar(long? id)
        {
            ...
            return View(chave.ChaveParticipantes.Where(c => c.Ativo).OrderBy(c => c.Colocacao).ToList());
        }
